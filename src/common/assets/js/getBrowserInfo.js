 ...
 * @Author: lize 
 * @Date: 2019/9/24 
 * @Description : 判断浏览器类型及版本 opera 、chrome、safari、msie、mozilla
 * @Parames : 
 * @Example : this.$getBrowserINfo();
 * @Last Modified by: lize
 * @Last Modified time: 2019/9/24
 */
export default function getBrowserInfo () {      //检测浏览器内核--返回的是两个key，name：浏览器内核的名称---version：浏览器的版本号
    var _broswer = {};
    ...
    var isIE = (sUserAgent.indexOf("compatible") > -1
        && sUserAgent.indexOf("MSIE") > -1)||(sUserAgent.indexOf('Trident') > -1 && sUserAgent.indexOf('rv:11.0') > -1
    ) && !isOpera;
    if (isIE) {
        var reIE = new RegExp("MSIE (\\d+\\.\\d+);");
        reIE.test(sUserAgent);
        console.log(RegExp['$1'],'RegExp[\'$1\']RegExp[\'$1\']RegExp[\'$1\']RegExp[\'$1\']')
        _broswer.version = parseFloat(RegExp['$1']) || 11 ;
        _broswer.msie = true;
        _broswer.name = 'msie';
    }
    // 排除Chrome 及 Konqueror/Safari 的伪装
    var isMoz = sUserAgent.indexOf("Gecko") > -1 && sUserAgent.indexOf('Firefox') > -1
        && !isChrome && !isKHTML;
    if (isMoz) {
    ...
    console.log(_broswer,'_broswer_broswer_broswer')
    return _broswer;
}